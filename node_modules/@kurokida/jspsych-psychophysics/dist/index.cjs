        ...
        this.img.onload = () => {
          ...
          if (this.mask === "gauss") {
            ...
            if (this.method === "ml-matrix") {
              const matrix_x = new mlMatrix.Matrix(coord_matrix_x);
              const matrix_y = new mlMatrix.Matrix(coord_matrix_y).transpose();
              const x_factor = mlMatrix.Matrix.pow(matrix_x, 2).mul(-1);
              const y_factor = mlMatrix.Matrix.pow(matrix_y, 2).mul(-1);
              const varScale = 2 * Math.pow(adjusted_sc, 2);
              exp_value = mlMatrix.Matrix.add(
                mlMatrix.Matrix.divide(x_factor, varScale),
                mlMatrix.Matrix.divide(y_factor, varScale)
              ).exp().to2DArray();
            }
            let cnt = 3;
            for (let i = 0; i < gauss_width; i++) {
    ...
    class gabor_stimulus extends visual_stimulus {
      ...
      update_position(elapsed) {
        ...
        if (this.method === "ml-matrix") {
          ...
          const x_factor = mlMatrix.Matrix.pow(matrix_x, 2).mul(-1);
          const y_factor = mlMatrix.Matrix.pow(matrix_y, 2).mul(-1);
          const sinWave = mlMatrix.Matrix.add(
            mlMatrix.Matrix.multiply(matrix_x, a),
            mlMatrix.Matrix.multiply(matrix_y, b)
          ).add(phase_rad).sin();
          const varScale = 2 * Math.pow(adjusted_sc, 2);
          const exp_value = this.disableGauss ? 1 : mlMatrix.Matrix.add(
            mlMatrix.Matrix.divide(x_factor, varScale),
            mlMatrix.Matrix.divide(y_factor, varScale)
          ).exp();
          const tmp1 = mlMatrix.Matrix.multiply(sinWave, exp_value);
          const tmp2 = mlMatrix.Matrix.multiply(tmp1, multConst);
          const tmp3 = mlMatrix.Matrix.multiply(
            mlMatrix.Matrix.multiply(tmp2, this.contrastPreMultiplicator),
            this.contrast
          );
          const m = mlMatrix.Matrix.multiply(mlMatrix.Matrix.add(tmp3, 0.5), 256);
...
module.exports = PsychophysicsPlugin;
